import pytest
from src.main import BadClass, viking_cafe_order, compute_other_thing  # Замените на актуальное имя модуля

def test_bad_class_get_value():
    bad_class_instance = BadClass()
    result = bad_class_instance.get_value()
    assert result == "some_other_value", f"Expected 'some_other_value', but got {result}"
